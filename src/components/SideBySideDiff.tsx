...
export default function SideBySideDiff() {
  ...
  const lineNumStyle = (type: DiffLine['type']) => {
    const base = 'select-none text-right pr-3 pl-2 w-12 shrink-0 border-r text-xs leading-7';
    if (type === 'empty') {
      return `${base} ${theme === 'dark' ? 'bg-zinc-900 border-zinc-800 text-zinc-700' : 'bg-zinc-100 border-zinc-200 text-zinc-300'}`;
    }
    if (type === 'added') {
      return `${base} ${theme === 'dark' ? 'bg-green-950/60 border-green-800/40 text-green-600' : 'bg-green-100/80 border-green-200 text-green-600'}`;
    }
    if (type === 'removed') {
      return `${base} ${theme === 'dark' ? 'bg-red-950/60 border-red-800/40 text-red-600' : 'bg-red-100/80 border-red-200 text-red-600'}`;
    }
  ...
  const lineContentStyle = (type: DiffLine['type']) => {
    const base = 'px-3 flex-1 min-w-0 whitespace-pre font-mono text-sm leading-7 overflow-x-auto';
    if (type === 'empty') {
      return `${base} ${theme === 'dark' ? 'bg-zinc-900/30' : 'bg-zinc-50'}`;
    }
    if (type === 'added') {
      return `${base} ${theme === 'dark' ? 'bg-green-950/25 text-green-200 border-l-2 border-l-green-500/50' : 'bg-green-50 text-green-900 border-l-2 border-l-green-500/50'}`;
    }
    if (type === 'removed') {
      return `${base} ${theme === 'dark' ? 'bg-red-950/25 text-red-200 border-l-2 border-l-red-500/50' : 'bg-red-50 text-red-900 border-l-2 border-l-red-500/50'}`;
    }
